...
var max_speed: float = 6 # Meters per second
var max_air_speed: float = 0.6
var accel: float = 60 # or max_speed * 10 : Reach max speed in 1 / 10th of a second
var friction: float = 2 # Higher friction = less slippery. In quake-based games, usually between 1 and 5
var max_ramp_angle: float = 45 # Max angle that the player can go upwards at full speed
var gravity: float = 15
var jump_impulse: float = 4.8
...
#	velocity = move_and_slide_with_snap(nextVelocity, snap, Vector3.UP, true, 4, deg_to_rad(max_ramp_angle))
...
func move_air(input_velocity: Vector3, delta: float)-> void:
	...
	# Then get back our vertical component, and move the player
	nextVelocity.y = vertical_velocity
#	velocity = move_and_slide(nextVelocity, snap, Vector3.UP)
